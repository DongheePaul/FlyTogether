...
public class GetCredentials {
    ...
    public void FromFile(String walletFile, String passwordWallet) throws IOException, CipherException {
        Log.e("walletFile in GetCredentials", walletFile);
        Log.e("password in GetCredentials", passwordWallet);
        Log.e("walletFile.length()", String.valueOf(walletFile.length()));
    ...
    private class LoadWalletFile extends AsyncTask<String,Void,Credentials>{
        ...
        protected Credentials doInBackground(String... values) {
            //File walletFile = new File("/data/user/0/info.bcdev.easytoken/files");

            Log.e("value[0] in LoadWallfetFile from GetCredentials", String.valueOf(values[0]));
            File walletFile = new File(values[0]);

            try {
                Log.e("value[1] in LoadWallfetFile from GetCredentials", String.valueOf(values[1]));
