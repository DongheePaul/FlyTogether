...
import com.dong.streaming.rtc_peer.kurento.KurentoPresenterRTCClient;
import com.dong.streaming.rtc_peer.kurento.models.CandidateModel;
import com.dong.streaming.rtc_peer.kurento.models.response.ServerResponse;
import com.dong.streaming.rtc_peer.kurento.models.response.TypeResponse;
import com.dong.streaming.util.RxScheduler;
...
public class BroadCasterPresenter extends MvpBasePresenter<BroadCasterView>
    ...
    private static final String STREAM_HOST = "wss://13.209.238.21:8443/one2many";
    ...
    //쿠렌토서버로 보내는 json Object.
    public void send_bj_id(String roomd_id, String filename, String bj_name) {
        try {
            Log.e("send_room_id", "들어옴 :"+roomd_id);
            Log.e("filename", "들어옴 :"+ filename);

            JSONObject obj = new JSONObject();
            //서버에서 id 값에 따라 어떻게 처리하는지 달라짐. id = "one"일때 roomid 값을 프리젠터의 고유 인덱스 값으로 사용.
            obj.put("id", "one");
            obj.put("name", roomd_id);
            obj.put("filename", filename);
            obj.put("bjname", bj_name);
    ...
    //쿠렌토서버로 bj가 방종했음을 알림.
    public void send_bj_stop(String roomd_id, String filename ) {
        try {
            Log.e("send_bj_stop room_id", "들어옴 :"+roomd_id);
            Log.e("send_bj_stop filename", "들어옴 :"+ filename);

            JSONObject obj = new JSONObject();
            //서버에서 id 값에 따라 어떻게 처리하는지 달라짐. id = "one"일때 roomid 값을 프리젠터의 고유 인덱스 값으로 사용.
            obj.put("id", "presenterStop");
            obj.put("name", roomd_id);
            obj.put("filename", filename);
